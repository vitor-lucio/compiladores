...
exp:  
      exp MAIS exp                                                  {                                                            
                                                                        $$.node = inicializa_node(constroi_codigo_abstrato_binop("MAIS"),$1.node, $3.node, NULL, constroi_codigo_intermediario_binop($2));
                                                                        $$.node->tipo = testinho(($1.node)->nome, ($3.node)->nome, ($1.node)->tipo, ($3.node)->tipo);     
                                                                        $$.node->nome = $$.node->tipo;                                                                     
                                                                        // printf("exp -> exp + exp\n"); 
                                                                    } /* BINOP(MAIS, exp, exp) */
    | exp MENOS exp                                                 {
                                                                        $$.node = inicializa_node(constroi_codigo_abstrato_binop("MENOS"),$1.node, $3.node, NULL, constroi_codigo_intermediario_binop($2));
                                                                        $$.node->tipo = testinho(($1.node)->nome, ($3.node)->nome, ($1.node)->tipo, ($3.node)->tipo);     
                                                                        $$.node->nome = $$.node->tipo;                                                                       
                                                                        // printf("exp -> exp - exp\n"); 
                                                                    } /* BINOP(MENOS, exp, exp) */
    | exp MULTIPLICACAO exp                                         {
                                                                        $$.node = inicializa_node(constroi_codigo_abstrato_binop("MULT"),$1.node, $3.node, NULL, constroi_codigo_intermediario_binop($2));
                                                                        $$.node->tipo = testinho(($1.node)->nome, ($3.node)->nome, ($1.node)->tipo, ($3.node)->tipo);     
                                                                        $$.node->nome = $$.node->tipo;       
                                                                        // printf("exp -> exp * exp\n"); 
                                                                    } /* BINOP(MULTIPLICACAO, exp, exp) */
    | exp DIVISAO exp                                               {
                                                                        $$.node = inicializa_node(constroi_codigo_abstrato_binop("DIV"),$1.node, $3.node, NULL, constroi_codigo_intermediario_binop($2));
                                                                        $$.node->tipo = testinho(($1.node)->nome, ($3.node)->nome, ($1.node)->tipo, ($3.node)->tipo);     
                                                                        $$.node->nome = $$.node->tipo;                                                                        
                                                                        // printf("exp -> exp / exp\n"); 
    ...
    | MENOS exp                                                     { 
                                                                        $$.node = inicializa_node("abstacthaha\n",$2.node, NULL, NULL, "TODO");
                                                                        // printf("exp -> - exp\n"); 
                                                                    } /* CONST(- exp) */
    | NUMERO                                                        {
                                                                        $$.node = inicializa_node(constroi_codigo_abstrato_const($1),NULL, NULL, NULL, constroi_codigo_intermediario_const($1));
                                                                        $$.node->tipo = "int";
                                                                        $$.node->nome = "int";
                                                                        $$.node->num_arrays = 0;
                                                                        // printf("exp -> num\n");
    ...
    | STRING_CONSTANTE                                              { 
                                                                        $$.node = inicializa_node(constroi_codigo_abstrato_constStr($1),NULL, NULL, NULL, constroi_codigo_intermediario_const($1));
                                                                        $$.node->tipo = "string";
                                                                        $$.node->nome = "string";
                                                                        // printf("exp -> string\n"); 
                                                                    } /* 'STRING_CONSTANTE' */
    ...
    | type_id ABRE_COLCHETE exp FECHA_COLCHETE OF exp               { 
                                                                        ...
                                                                        if(expr){                                                                            
                                                                            ...
                                                                            $$.node = inicializa_node(constroi_codigo_abstrato_call3(),$1.node, $3.node, $6.node, constroi_codigo_intermediario_call3(expr->temp));
                                                                        }else{
                                                                            ...
                                                                            $$.node = inicializa_node(constroi_codigo_abstrato_call3(),$1.node, $3.node, $6.node, constroi_codigo_intermediario_call1());
                                                                        }
    ...
    | type_id                                                       { 
                                                                        $$.node = inicializa_node(constroi_codigo_abstrato_type_id(),$1.node, NULL, NULL, constroi_codigo_intermediario_type_id()); 
    ...
    | LET decs IN expseq END                                        { 
                                                                        $$.node = inicializa_node(constroi_codigo_abstrato_eseq(),$2.node, $4.node, NULL, constroi_codigo_intermediario_eseq());
...
type_id: 
      VARIAVEL  {   
                    ...
                    if(simbolo_encontrado){
                        // printf("OLAAA\n");
                        $$.node = inicializa_node(constroi_codigo_abstrato_temp($1),NULL, NULL, NULL, constroi_codigo_intermediario_temp(simbolo_encontrado->temp));                                                        
                    }else{
                        $$.node = inicializa_node(constroi_codigo_abstrato_temp($1),NULL, NULL, NULL, constroi_codigo_intermediario_temp(pega_ultimo_temp_de_todos()));  
                    }
...
l_value: 
    ...
    | type_id ABRE_COLCHETE exp FECHA_COLCHETE          { 
                                                            $$.node = inicializa_node("abstacthaha\n",$1.node, $3.node, NULL, constroi_codigo_intermediario_mem());
                                                            // printf("l-value -> type-id [ exp ]\n"); 
                                                        }
    | l_value ABRE_COLCHETE exp FECHA_COLCHETE          { 
                                                            $$.node = inicializa_node("abstacthaha\n",$1.node, $3.node, NULL, "TODO");
...
expseq: 
      exp expseq1           {    
                                $$.node = inicializa_node(constroi_codigo_abstrato_expseq(),$1.node, $2.node, NULL, constroi_codigo_intermediario_expseq());
                                // printf("expseq -> exp expseq1\n"); 
                            } /* Nenhum código intermediário neste nó */
    |                       {   
                                $$.node = inicializa_node(constroi_codigo_abstrato_vazio(),NULL, NULL, NULL, constroi_codigo_intermediario_vazio());
                                // printf("expseq -> \n"); 
                            } /* Nenhum código intermediário neste nó */
    ;
...
expseq1: 
      PONTO_E_VIRGULA exp expseq1               { 
                                                    $$.node = inicializa_node("abstacthaha\n",$2.node, $3.node, NULL, constroi_codigo_intermediario_expseq1());
                                                    // printf("expseq1 -> ; exp expseq1\n"); 
                                                } /* Nenhum código intermediário neste nó */
    |                                           { 
                                                    $$.node = inicializa_node("abstacthaha\n",NULL, NULL, NULL, constroi_codigo_intermediario_vazio());
                                                    // printf("expseq1 -> \n"); 
                                                } /* Nenhum código intermediário neste nó */
    ;
...
args: 
      exp args1                 { 
                                    $$.node = inicializa_node("abstacthaha\n",$1.node, $2.node, NULL, constroi_codigo_intermediario_args());                                                            
...
args1: 
      VIRGULA exp args1             { 
                                        $$.node = inicializa_node("abstacthaha\n",$2.node, $3.node, NULL, constroi_codigo_intermediario_args1());
...
tyfields: 
      VARIAVEL DOIS_PONTOS type_id tyfields1            {   
                                                            $$.node = inicializa_node(constroi_codigo_abstrato_tyfields($1),$3.node, $4.node, NULL, constroi_codigo_intermediario_tyfields());
                                                            ...
                                                            */
                                                            if(!strcmp(simbolo_encontrado->classe, "?")){
                                                                ...
                                                                simbolo_encontrado->numero_de_parametros = numero_do_param_atual;
                                                                ...
                                                                numero_do_param_atual++;
                                                                novo_simbolo->numero_de_parametros = numero_do_param_atual;
    ...
    |                                                   { 
                                                            $$.node = inicializa_node(constroi_codigo_abstrato_tyfields12(),NULL, NULL, NULL, constroi_codigo_intermediario_tyfields2());
...
tyfields1: 
      VIRGULA VARIAVEL DOIS_PONTOS type_id tyfields1        { 
                                                                $$.node = inicializa_node(constroi_codigo_abstrato_tyfields11($2),$4.node, $5.node, NULL, constroi_codigo_intermediario_tyfields11());
                                                                // printf("------------- Node de ', variavel : tipo' -------------\n");
    ...
    |                                                       { 
                                                                $$.node = inicializa_node(constroi_codigo_abstrato_tyfields12(),NULL, NULL, NULL, constroi_codigo_intermediario_tyfields12());
                                                                $$.node->numero_de_parametros = 0;
                                                                // printf("tyfields1 -> \n"); 
                                                            }
    ;
...
ty: 
      VARIAVEL                                                          {  
                                                                            $$.node = inicializa_node(constroi_codigo_abstrato_ty1($1),NULL, NULL, NULL, constroi_codigo_intermediario_ty1()); 
                                                                            $$.node->tipo = $1;                                                                           
                                                                            // printf("ty -> id\n"); 
                                                                        }
    ...
    | ARRAY OF VARIAVEL                                                 { 
                                                                            $$.node = inicializa_node(constroi_codigo_abstrato_ty3($3),NULL, NULL, NULL, constroi_codigo_intermediario_ty3());                                                        
                                                                            $$.node->tipo = "array";
      ...
      VAR VARIAVEL ATRIBUICAO exp                       {                                                             
                                                            ...
                                                            */
                                                            ...
                                                            if(!strcmp(simbolo_encontrado->classe, "?")){
                                                                ...
                                                                $$.node = inicializa_node(constroi_codigo_abstrato_vardec1($2),$4.node, NULL, NULL, constroi_codigo_intermediario_vardec1(simbolo_encontrado->temp));                                                        
                                                            }
                                                            else{
                                                                struct simbolo* novo_simbolo = inicializa_simbolo(simbolo_encontrado->nome, busca_e_define_tipo(($4.node)->tipo), "?", CLASSE_VARIAVEL, "?", "?");
                                                                adiciona_simbolo_sem_verificacoes(novo_simbolo);
                                                                $$.node = inicializa_node(constroi_codigo_abstrato_vardec1($2),$4.node, NULL, NULL, constroi_codigo_intermediario_vardec1(novo_simbolo->temp));  
                                                            }                                                                                                                                                                                   
    ...
    | VAR VARIAVEL DOIS_PONTOS type_id ATRIBUICAO exp   {  
                                                            ...
                                                            */
                                                            ...
                                                            if(!strcmp(simbolo_encontrado->classe, "?")){
                                                                ...
                                                                $$.node = inicializa_node(constroi_codigo_abstrato_vardec2(simbolo_encontrado->nome),$4.node, $6.node, NULL, constroi_codigo_intermediario_vardec2(simbolo_encontrado->temp));                                                        
                                                            }else{
                                                                ...
                                                                adiciona_simbolo_sem_verificacoes(novo_simbolo);
                                                                $$.node = inicializa_node(constroi_codigo_abstrato_vardec2(novo_simbolo->nome),$4.node, $6.node, NULL, constroi_codigo_intermediario_vardec2(novo_simbolo->temp));  
                                                            }
                                                            /**/                                                                                                                           
...
fundec: 
      FUNCTION VARIAVEL ABRE_PARENTESES tyfields FECHA_PARENTESES IGUAL exp                     { 
                                                                                                    $$.node = inicializa_node(constroi_codigo_abstrato_fundec2($2),$4.node, $7.node, NULL, constroi_codigo_intermediario_fundec2());
    ...
    | FUNCTION VARIAVEL ABRE_PARENTESES tyfields FECHA_PARENTESES DOIS_PONTOS type_id IGUAL exp {                                                                                                     
                                                                                                    $$.node = inicializa_node(constroi_codigo_abstrato_fundec1($2),$4.node, $7.node, $9.node, constroi_codigo_intermediario_fundec1()); 
                                                                                                    numero_do_param_atual = 0;
...
decs: 
      dec decs          {   
                            if(($1.node)->dec)
                                $$.node = inicializa_node(constroi_codigo_abstrato_decs(),$1.node, $2.node, NULL, constroi_codigo_intermediario_decs1_vazio()); 
                            else
                                $$.node = inicializa_node(constroi_codigo_abstrato_decs(),$1.node, $2.node, NULL, constroi_codigo_intermediario_decs1()); 
                            // printf("decs -> dec decs\n"); 
                        }
    |                   { 
                            $$.node = inicializa_node(constroi_codigo_abstrato_decs2(),NULL, NULL, NULL, constroi_codigo_intermediario_decs2());
                            $$.node->dec = -1;
                            // printf("decs -> \n"); 
                        } /* Nenhum código intermediário neste nó */
    ;
...
dec: 
      tydec         { 
                        $$.node = inicializa_node(constroi_codigo_abstrato_dec_tydec(),$1.node, NULL, NULL, constroi_codigo_intermediario_dec_tydec());
                        $$.node->dec = 1;
                        // printf("dec -> tydec\n"); 
                    }
    | vardec        { 
                        $$.node = inicializa_node(constroi_codigo_abstrato_dec_vardec(),$1.node, NULL, NULL, constroi_codigo_intermediario_dec_vardec());                                                           
                        $$.node->dec = 0;
                        // printf("dec -> vardec\n"); 
                    }
    | fundec        {                             
                        $$.node = inicializa_node(constroi_codigo_abstrato_dec_fundec(),$1.node, NULL, NULL, constroi_codigo_intermediario_dec_fundec());                                                        
                        $$.node->dec = 1;                       
                        // printf("dec -> fundec\n"); 
                    }
    ;
...
int main(int argc, char** argv) {
    ...
    codigo_abstrato_final = monta_codigo_abstrato_da_arvore(raiz_da_arvore);
    printf("\n%s\n", codigo_abstrato_final);
